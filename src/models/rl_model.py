...
# Initialiser et entrainer le modèle de régression logistique
logreg = LogisticRegression(solver='lbfgs', multi_class='auto', max_iter=1000)
logreg.fit(hog_descriptors, labels)
...
y_pred = logreg.predict(hog_descriptors_test)

# Calculer la précision
accuracy = accuracy_score(labels_test, y_pred)
print(f"La précision du modèle de régression logistique est : {accuracy}")
